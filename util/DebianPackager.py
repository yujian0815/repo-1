...
class DebianPackager(object):
    ...
    def CompileControl(self, tweak_data, repo_settings):
        ...
        control_file += "Depiction: https://" + repo_settings['cname'] + subfolder + "/depiction/web/" + tweak_data['bundle_id'] \
                        + ".html\n"
        control_file += "Homepage: https://" + repo_settings['cname'] + subfolder + "/depiction/web/" + tweak_data['bundle_id'] \
            + ".html\n"
        ...
        try:
            if tweak_data['Support']:
                control_file += "Support: " + tweak_data['Support'] + "\n"
        except Exception:
            control_file += "描述：支持不支持!\n"
        
        try:
            if tweak_data['Installed-Size']:
                control_file += "Installed-Size: " + tweak_data['Installed-Size'] + "\n"
        except Exception:
                ...
                if not os.path.isdir(self.root + "Packages/" + folder + "/silica_data"):
                    ...
                    try:
                        ...
                        print("\033[91m- 配置错误! -\n"
                              ...
                              " 应该由多个文件夹组成，每个文件夹包含单个包的数据.\n"
                              "请解决此问题并重试.\033[0m")
                        quit()

                    # T他的将是我们包的默认脚手架。最后，我会把它阉割成
                    # 基本要素；也有点像是对我的参考.
                    output = {
                    ...
                    if is_deb:
                        ...
                        try:
                            output['Installed-Size'] = deb.headers['Installed-Size']
                        except Exception:
                            output['Installed-Size'] = input("包裹的大小 ")
                        try:
                        ...
                        output['developer']['email'] = "409774743@qq.com"
                        try:
